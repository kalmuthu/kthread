...
int save_context(jmp_buf *ctx, THREAD *t, void *arg)
{
...
int restore_context(jmp_buf *ctx, int value)
{
        register int eax asm("eax");
        register int ebx asm("ebx");
        register int ecx asm("ecx");
...
void timer_interrupt(){
    ...
    if(save_context(&current->ctx, current, current->arg) != 0){
        return;
    }
        current = current->next;
        printf("Switching to Thread: %s\n", current->name);
        restore_context(&current->ctx,1);
